...
def analyze_remaining_opportunities():
    ...
    if len(fuzzy_df) > 0:
        ...
        if low_conf > 0:
            ...
            for (model, canonical), count in fuzzy_patterns.most_common(10):
                avg_score = low_fuzzy[(low_fuzzy["model"] == model) & (low_fuzzy["canonical_model"] == canonical)][
                    "match_score"
                ].mean()
    ...
    amd_fps = df[df["title"].str.contains("AMD|Radeon|RX", case=False, na=False) & (df["canonical_model"] != "UNKNOWN")]
    intel_fps = df[df["title"].str.contains("Intel|Arc", case=False, na=False) & (df["canonical_model"] != "UNKNOWN")]
    ...
    if len(amd_fps) > 0 or len(intel_fps) > 0:
        opportunities.append("Fix remaining cross-vendor false positives")
