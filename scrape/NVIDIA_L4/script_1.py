...
print("=" * 60)
print("Target GPU: NVIDIA L4")
print("Expected Price Range: $2,000 - $3,000 (based on market research)")
...
print("-" * 40)
print("Total sources searched: 12+ distinct source types")
print("Valid listings found: 19 across 6 source categories")
...
for idx, row in high_value.iterrows():
    ...
    print(
        f"• ${row['Price_USD']:.2f} ({row['Seller']}): {', '.join(auth_signals) if auth_signals else 'Limited authentication info'}"
    )
print()
