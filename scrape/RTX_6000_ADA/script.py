...
for idx, row in df.iterrows():
    if row['Price_USD'] < mean_price * 0.6:
        print(f"⚠️  LOW: ${row['Price_USD']:,.2f} at {row['Seller']} - {40+((mean_price-row['Price_USD'])/mean_price*100):.0f}% below average")
    elif row['Price_USD'] > mean_price * 1.4:
        print(f"🔺 HIGH: ${row['Price_USD']:,.2f} at {row['Seller']} - {((row['Price_USD']-mean_price)/mean_price*100):.0f}% above average")
...
print(df_sorted[['Model', 'Condition', 'Price_USD', 'Seller', 'Source_Type', 'Bulk_Notes']].head(10).to_string(index=False))