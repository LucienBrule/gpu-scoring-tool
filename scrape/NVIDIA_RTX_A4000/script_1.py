...
for _, row in low_price_flags.iterrows():
    ...
    if row["Price_USD"] < 500:
        print("    ⚠️  WARNING: Price 50%+ below typical - may be liquidation/bulk/damaged")

print(f"\nUnusually HIGH prices (> ${high_threshold:.0f}):")
for _, row in high_price_flags.iterrows():
    print(f"  • ${row['Price_USD']:.2f} - {row['Seller']} ({row['Condition']}) - {row['Geographic_Region']}")
    if row["Price_USD"] > 1300:
        print("    ⚠️  WARNING: Price 30%+ above typical - may include premium warranty/service")
...
print("-" * 28)
geo_dist = (
    df[df["Price_USD"] > 0].groupby("Geographic_Region").agg({"Price_USD": ["count", "mean", "min", "max"]}).round(2)
)
