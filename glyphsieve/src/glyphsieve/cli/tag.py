...
def quantization(input, output, config_file):
    ...
    try:
        ...
        if "quantization_capable" in df.columns:
            capable_count = df[df["quantization_capable"] == True].shape[0]
            not_capable_count = df[df["quantization_capable"] == False].shape[0]
            ...
            table.add_row(
                "Quantization Capable",
                str(capable_count),
                f"{capable_count / total_count * 100:.1f}%"
            )
            table.add_row(
                "Not Capable",
                str(not_capable_count),
                f"{not_capable_count / total_count * 100:.1f}%"
            )
            table.add_row(
                "Total Rows",
                str(total_count),
            ...
            if capable_count > 0:
                ...
                example = df[df["quantization_capable"] == True].iloc[0]
            ...
            if not_capable_count > 0:
                ...
                example = df[df["quantization_capable"] == False].iloc[0]
                console.print(f"[cyan]Model:[/cyan] {example.get('canonical_model', 'N/A')}")
                console.print(f"[cyan]VRAM:[/cyan] {example.get('vram_gb', 'N/A')} GB")
                console.print(f"[cyan]TDP:[/cyan] {example.get('tdp_watts', 'N/A')} watts")
                console.print(f"[cyan]MIG Support:[/cyan] {example.get('mig_support', 'N/A')}")
