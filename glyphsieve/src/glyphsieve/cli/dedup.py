...
@click.option(
    ...
    "-p",
    type=float,
    default=DEFAULT_PRICE_EPSILON,
    help=f"Price difference tolerance as fraction (default: {DEFAULT_PRICE_EPSILON})",
)
@click.option(
    "--model", "-m", default="all-MiniLM-L6-v2", help="Sentence transformer model to use (default: all-MiniLM-L6-v2)"
)
def dedup(input, output, similarity, price_epsilon, model):
    ...
    try:
        ...
        df = dedup_csv(input, output, similarity_threshold=similarity, price_epsilon=price_epsilon, model_name=model)
