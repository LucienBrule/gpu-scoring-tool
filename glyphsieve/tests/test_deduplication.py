...
class TestDuplicateDetection:
        ...
        df = pd.DataFrame(
            {
                ...
                "seller": [
                    ...
                    "Seller A",
                    "Seller B",
                ],  # Same seller for first two to trigger duplicate detection
            }
        )

        # Mock the embeddings to simulate high similarity for the first two titles
        # and low similarity for the third
        similarity_matrix = np.array([[1.0, 0.96, 0.5], [0.96, 1.0, 0.5], [0.5, 0.5, 1.0]])
    ...
    def test_find_duplicates_identical_urls(self, mock_generate_embeddings):
        ...
        similarity_matrix = np.array([[1.0, 0.85], [0.85, 1.0]])
