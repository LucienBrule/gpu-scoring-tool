...
def test_weighted_additive_scorer():
    ...
    row = {"vram_gb": 80, "mig_support": 7, "nvlink": True, "tdp_watts": 350, "price": 5000.0}
...
def test_score_listing_edge_cases():
    ...
    # Test zero values
    row_zero_values = {"vram_gb": 0, "mig_support": 0, "nvlink": False, "tdp_watts": 0, "price": 0}
    score = scorer.score_listing(row_zero_values, weights)
    assert 0.0 <= score <= 1.0

    # Test negative values (should be handled gracefully)
    row_negative_values = {"vram_gb": -10, "mig_support": -1, "nvlink": False, "tdp_watts": -200, "price": -1000}
