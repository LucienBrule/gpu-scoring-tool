    ...
    for slot in sorted(slot_widths):
        ...
        if len(subset) > 0:
            ...
            for _, row in subset.iterrows():
                price_formatted = (
                    f"${row['Current_Price']/1000:.1f}k"
                    if row["Current_Price"] < 1000000
                    else f"${row['Current_Price']/1000000:.1f}m"
                )
                hover_text.append(
            ...
            fig.add_trace(
                go.Scatter3d(
                    ...
                    mode="markers",
                    marker=dict(
                        color=color_map[mig], symbol=symbol_map[slot], size=8, line=dict(width=1, color="white")
                    ),
